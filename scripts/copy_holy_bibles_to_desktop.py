...
def place(src: Path, dest: Path, force: bool = False) -> str:
...
These files are US Letter. Print at 100% / Actual size (do not scale). Page 2 is a clickable chapter glossary. Every page already has {SITE} and the maker line. Scripture copyright stays with each translation’s publishers and is printed on the cover.
"""
    (dest / "Read Me.txt").write_text(text, encoding="utf-8")


def organize(dest: Path, source: Path | None = None, force: bool = False, language: str = "") -> dict:
    source = source or PDF_DIR
    rows = load_catalog()
    if language:
        want = language.casefold()
        rows = [row for row in rows if (row.get("language") or "").casefold() == want]
    ...
    index_rows = []
    for row in rows:
        language_name = safe_name(row.get("language") or "Unknown", "Unknown")
        bucket = letter_bucket(language_name)
        used = used_by_lang.setdefault(language_name, set())
        filename = pdf_filename(row, used)
        src = source / f"ChristSupplyHolyBible-{row['id']}.pdf"
        if not src.exists():
            missing.append(row["id"])
            continue
        target = dest / bucket / language_name / filename
        place(src, target, force=force)
        placed += 1
        index_rows.append(
            {
                "letter": bucket,
                ...
                "id": row["id"],
                "file": str(Path(bucket) / language_name / filename),
            }
        )
    ...
    if not language:
        for sample in sorted(SAMPLE_DIR.glob("ChristSupply.Net-*.pdf")):
            place(sample, sample_dir / sample.name, force=force)
...
def main() -> None:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite existing PDFs in the destination")
    parser.add_argument("--language", default="", help="Only copy this language, e.g. English")
    args = parser.parse_args()
    dest = Path(args.dest).expanduser() if args.dest else default_desktop()
    dest.parent.mkdir(parents=True, exist_ok=True)
    if args.clean and dest.exists():
        shutil.rmtree(dest)
    result = organize(dest, Path(args.source), force=args.force, language=args.language)
